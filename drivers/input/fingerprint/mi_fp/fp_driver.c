...
static struct fp_key_map maps[] = {
	{ EV_KEY, FP_KEY_INPUT_HOME },	 { EV_KEY, FP_KEY_INPUT_MENU },
	{ EV_KEY, FP_KEY_INPUT_BACK },	 { EV_KEY, FP_KEY_INPUT_POWER },
...
static void fp_screen_state_for_fingerprint_callback(
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	if (notifier_tag == PANEL_EVENT_NOTIFICATION_PRIMARY ||
	    notifier_tag == PANEL_EVENT_NOTIFICATION_SECONDARY) {
...
static void fp_register_panel_notifier_work(struct work_struct *work)
{
	struct fp_device *fp_dev =
		container_of(work, struct fp_device, screen_state_dw.work);
	int error = 0;
	...
	if (prim_panel) {
		pr_info("success to get primary panel, retry times = %d",
			retry_count);
		if (!cookie) {
			cookie = panel_event_notifier_register(
				...
				prim_panel,
				fp_screen_state_for_fingerprint_callback,
				(void *)fp_dev);
			if (IS_ERR(cookie))
		...
		if (retry_count++ < 5) {
			queue_delayed_work(fp_dev->screen_state_wq,
					   &fp_dev->screen_state_dw, 5 * HZ);
		} else {
...
#if IS_ENABLED(CONFIG_FP_HAVE_MULTI_SCREEN)
	if (sec_panel) {
		pr_info("success to get second panel, retry times = %d",
			retry_count);
		if (!cookie_sec) {
				    ...
				    const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", strlen("1"))) {
		...
		sysfs_notify(&fp_dev->driver_device->dev.kobj, NULL,
			     "fingerdown");
...
static DEVICE_ATTR(fingerdown, S_IRUSR | S_IWUSR, get_fingerdown_event,
		   set_fingerdown_event);
#endif

static struct attribute *attributes[] = { &dev_attr_irq.attr,
#ifdef CONFIG_SIDE_FINGERPRINT
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_IOC_ENABLE_POWER:
		...
		if (fp_dev->device_available == 1) {
			...
			if (copy_from_user((char *)vendor_name, (char *)arg,
					   10 * sizeof(char))) {
			...
			if (!strcmp(vendor_name, "goodix"))
				fp_dev->vreg = regulator_get(
					&fp_dev->driver_device->dev,
					DTS_VOlT_REGULATER_GD);
			else if (!strcmp(vendor_name, "fpc"))
				fp_dev->vreg = regulator_get(
					&fp_dev->driver_device->dev,
					DTS_VOlT_REGULATER_FPC);
			else {
				pr_err("%s is unknown vendor name",
				       vendor_name);
				return -EPERM;
			}
...
#ifndef XIAOMI_DRM_INTERFACE_WA
	fp_dev->screen_state_wq =
		create_singlethread_workqueue("screen_state_wq");
	if (fp_dev->screen_state_wq) {
		INIT_DELAYED_WORK(&fp_dev->screen_state_dw,
				  fp_register_panel_notifier_work);
	}
#endif
...
#else
	status = sysfs_create_group(&fp_dev->driver_device->dev.kobj,
				    &attribute_group);
	if (status) {
