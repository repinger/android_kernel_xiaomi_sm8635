...
static int adspsleepmon_smem_init(void)
{
	...
	if (IS_ERR_OR_NULL(g_adspsleepmon.lpm_stats) ||
		...
		pr_err("Failed to get sleep stats from SMEM for ADSP: %ld, size: %zu\n",
	...
	if (IS_ERR_OR_NULL(g_adspsleepmon.lpi_stats) ||
		(sizeof(struct sleep_stats) > size)) {
		pr_err("Failed to get LPI sleep stats from SMEM for ADSP: %ld, size: %zu\n",
	...
	if (IS_ERR_OR_NULL(g_adspsleepmon.dsppm_stats) ||
		...
		pr_err("Failed to get DSPPM stats from SMEM for ADSP: %ld, size: %zu\n",
	...
	if (IS_ERR_OR_NULL(stats) || !size) {
		pr_err("Failed to get SysMon stats from SMEM for ADSP: %ld, size: %zu\n",
				PTR_ERR(stats), size);
		return -ENOMEM;
	}
...
static void print_complete_dsppm_info(void)
{
	...
	if (g_adspsleepmon.dsppm_clients.result ==
		...
		for (i = 0; i < g_adspsleepmon.dsppm_clients.num_clients; i++) {
			pr_err("%u:N:%s,ID:%u,RTS:%llu,TMIPS:%u,MIPSPT:%u,TMPPS:%u,ADSPFCLK:%u,BPS:%llu,UP:%u,SL:%u,POW:%u,PD:%s,ISP:%u\n",
