...
#define wl2866d_vdd_step(type) (type ? 12500 : 6000)
...
static int wl2866d_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	rc = wl2866d_read(wl2866d_reg->regmap, wl2866d_reg->addr, &reg_voltage,
			  1);
	if (rc < 0) {
...
static int wl2866d_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (current_uv < 0) {
		wl2866d_err(wl2866d_reg,
			    "failed to get current voltage rc = %d\n",
			    current_uv);
		return current_uv;
	}
				 ...
				 int min_uv, int max_uv)
{
	...
	rc = wl2866d_write(wl2866d_reg->regmap, wl2866d_reg->addr, &reg_voltage,
			   1);
	if (rc < 0) {
				...
				const char *name)
{
	...
	if (rc < 0) {
		wl2866d_err(wl2866d_reg,
			    "failed to get regulator index rc = %d\n", rc);
		return rc;
	}
	rc = of_property_read_u8(of_node, "reg", &wl2866d_reg->addr);
	if (rc < 0) {
		wl2866d_err(wl2866d_reg,
			    "failed to get regulator register rc = %d\n", rc);
		return rc;
	}
	rc = of_property_read_string(of_node, "type", &vdd_type);
	if (rc < 0) {
		wl2866d_err(wl2866d_reg,
			    "failed to get regulator type rc = %d\n", rc);
		return rc;
	}
	...
	init_data =
		of_get_regulator_init_data(dev, of_node, &wl2866d_reg->rdesc);
	if (init_data == NULL) {
			      ...
			      const struct i2c_device_id *id)
{
	...
	chip->dev = &client->dev;
	chip->regmap =
		devm_regmap_init_i2c(client, &wl2866d_regulator_regmap_config);
	if (!chip->regmap)
		return -ENODEV;

	chip->pinctrl = devm_pinctrl_get(&client->dev);
	if (!IS_ERR_OR_NULL(chip->pinctrl)) {
		chip->gpio_state_enable = pinctrl_lookup_state(
			chip->pinctrl, WL2866D_PINCTRL_ENABLE);
		if (IS_ERR_OR_NULL(chip->gpio_state_enable)) {
			pr_err("wl2866d_pmic: failed to get wl2866d enabled pinctrl\n");
			return -EINVAL;
		}
		chip->gpio_state_disable = pinctrl_lookup_state(
			chip->pinctrl, WL2866D_PINCTRL_DISABLE);
	...
	for_each_available_child_of_node(chip->dev->of_node, child) {
		wl2866d_reg = devm_kzalloc(chip->dev, sizeof(*wl2866d_reg),
					   GFP_KERNEL);
		if (!wl2866d_reg)
			return -ENOMEM;
		...
		if (rc < 0) {
			wl2866d_err(wl2866d_reg,
				    "failed to read register name rc = %d\n",
				    rc);
			return rc;
		}
...
static void wl2866d_pmic_remove(struct i2c_client *client)
{
